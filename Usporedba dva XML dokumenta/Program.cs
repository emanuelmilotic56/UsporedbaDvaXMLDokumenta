...
namespace xml
{
    class Program
    {
        private void LinqUsporediDvaXMLDokumenta()
        {
            string StariXML = @"<Books>
    ...
    <book id='5' image='C05' name='PowerShell in Action'/>
            ...
            XDocument xmlStari = XDocument.Parse(StariXML);
            XDocument xmlNovi = XDocument.Parse(NoviXML);

            var res = (from b1 in xmlStari.Descendants("book")
                       from b2 in xmlNovi.Descendants("book")
            ...
            var res2 = (from b2 in xmlNovi.Descendants("book") select new XElement("book", new XAttribute("id", b2.Attribute("id").Value))).Except(xmlStari.Descendants("book"));
